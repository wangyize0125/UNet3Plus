...
from loss.diceloss import DiceLoss, FocalLoss
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc="Epoch {} / {}".format(epoch + 1, epochs), unit="image") as pbar:
            for batch in train_loader:
                ...
                criterion = FocalLoss(masks_pred, true_masks)
                focal_loss = criterion()
                epoch_loss += focal_loss.item()

                writer.add_scalar("Loss/train", loss.item() + focal_loss.item(), global_step + 1)
                pbar.set_postfix(**{"loss (batch)": loss.item() + focal_loss.item()})
